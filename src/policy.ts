...
export abstract class ResponsePolicy<
  Options extends Record<string, unknown> | undefined,
  ResponseType extends IResponse = IResponse,
  Args extends unknown[] = unknown[],
> {
...
> {
  private constructor(
    private readonly result: RequestType | ResponseType,
    private readonly type: PolicyResultType
  ) {}
  ...
  static request<RequestType extends IRequest = IRequest>(
    result: RequestType
  ): PolicyResult<RequestType, any> {
    return new PolicyResult(result, PolicyResultType.Request)
  }

  static response<ResponseType extends IResponse = IResponse>(
    result: ResponseType
  ): PolicyResult<any, ResponseType> {
